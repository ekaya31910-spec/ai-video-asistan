...
def call_llm(prompt: str) -> str:
    """Once Gemini API (ucretsiz, key gerekli) dener, olmazsa Pollinations'a (key gerekmez,
    ama kotasi cok dusuk) duser."""
    if GEMINI_API_KEY:
        try:
            resp = requests.post(
            ...
            st.warning(f"Gemini API hatasi, Pollinations deneniyor: {e}")
...
with st.sidebar:
    ...
    if GEMINI_API_KEY:
        st.success("Gemini bagli (senaryo yazimi)")
    else:
        st.error("Gemini key yok! Senaryo/sahne yazimi calismaz. Asagidaki rehbere bak.")
